...
final class LiDARCaptureEngine: NSObject, ObservableObject {
    ...
    // MARK: - 3D Export
    //
    // Actual writers live in LiDARExportWriters.swift (PR-B7). These
    // wrappers preserve the pre-existing call sites inside the engine.

    private func savePointsBinary(_ points: [SIMD3<Float>], to url: URL) async throws {
        try await LiDARExportWriters.savePointsBinary(points, to: url)
    }

    private func exportMeshToUSDZAsync(_ anchors: [ARMeshAnchor], to url: URL) async throws {
        try await LiDARExportWriters.exportMeshToUSDZAsync(anchors, to: url)
    }
}
